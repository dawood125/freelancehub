...
import {
  FiStar, FiClock, FiRefreshCw, FiCheck,
  FiMapPin, FiCalendar, FiMessageSquare,
...
const GigDetailPage = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/gigs" className="text-green-600 font-medium hover:text-green-700">
            ← Browse all services
          </Link>
        </div>
      </div>
  ...
  const packageTabs = [
    ...
    ...(packages.standard?.isActive ? [{ key: 'standard', label: 'Standard' }] : []),
    ...(packages.premium?.isActive ? [{ key: 'premium', label: 'Premium' }] : []),
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <Link to="/gigs" className="hover:text-green-600 transition-colors">Services</Link>
              <span>/</span>
            ...
            >
              <Link to={`/users/${seller.username}`} className="flex items-center gap-3 group">
                {seller.avatar?.url ? (
                  <img src={seller.avatar.url} alt={seller.name} className="w-10 h-10 rounded-full object-cover" />
                ) : (
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-bold text-amber-500">{gig.ratings?.average?.toFixed(1) || 'New'}</span>
                <span className="text-gray-400 text-sm">({gig.ratings?.count || 0} reviews)</span>
              </div>
            </motion.div>
                        ...
                        {images.map((img, idx) => (
                          <button
                            ...
                            className={`w-16 h-12 rounded-lg overflow-hidden border-2 transition-all ${
                              idx === currentImageIndex ? 'border-white shadow-lg scale-105' : 'border-transparent opacity-70 hover:opacity-100'
                            }`}
                          >
          ...
          <div className="space-y-6">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.15 }}
              className="bg-white rounded-2xl border border-gray-100 shadow-xl shadow-gray-200/50 overflow-hidden sticky top-24"
            >
              {/* Package Tabs */}
              <div className="flex border-b border-gray-100">
                {packageTabs.map(tab => (
                  <button
                    ...
                    className={`flex-1 py-4 text-sm font-bold text-center transition-all duration-300 relative ${
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600 mb-5">
                  <span className="flex items-center gap-1.5">
                    ...
                    {currentPkg.deliveryDays} day{currentPkg.deliveryDays !== 1 ? 's' : ''} delivery
                  </span>
                  <span className="flex items-center gap-1.5">
                    <FiRefreshCw className="w-4 h-4 text-gray-400" />
                    {currentPkg.revisions === -1 ? 'Unlimited' : currentPkg.revisions} revision{currentPkg.revisions !== 1 ? 's' : ''}
                  </span>
                </div>
                ...
                {/* Order Button */}
                <button className="w-full py-3.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold rounded-xl hover:shadow-lg hover:shadow-green-500/25 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 flex items-center justify-center gap-2">
                  Continue
                  <FiArrowRight className="w-5 h-5" />
                </button>

                {/* Contact Button */}
                <button className="w-full mt-3 py-3 text-gray-700 font-semibold border-2 border-gray-200 rounded-xl hover:border-green-500 hover:text-green-600 transition-all duration-300 flex items-center justify-center gap-2">
            ...
            >
              ...
              <Link to={`/users/${seller.username}`} className="flex items-center gap-4 mb-4 group">
                {seller.avatar?.url ? (
                  <img src={seller.avatar.url} alt={seller.name} className="w-16 h-16 rounded-2xl object-cover" />
                ) : (
                ...
                <div>
                  <h4 className="font-bold text-gray-900 group-hover:text-green-600 transition-colors">{seller.name}</h4>
                  {seller.title && <p className="text-sm text-gray-500">{seller.title}</p>}
                </div>
              </Link>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {[
                  { label: 'Rating', value: `${seller.freelancerProfile?.averageRating?.toFixed(1) || '0.0'} ⭐` },
                  { label: 'Reviews', value: seller.freelancerProfile?.totalReviews || 0 },
                  { label: 'Orders', value: seller.freelancerProfile?.completedOrders || 0 },
                  { label: 'Level', value: (seller.freelancerProfile?.level || 'new').replace('-', ' ') },
                ].map(stat => (
              ...
              <div className="space-y-2.5 text-sm">
                {seller.location?.country && (
                  <div className="flex items-center gap-2 text-gray-500">
                    ...
                    {seller.location.city && `${seller.location.city}, `}{seller.location.country}
                  </div>
                )}
                <div className="flex items-center gap-2 text-gray-500">
